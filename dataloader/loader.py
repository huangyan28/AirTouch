...
class loader(Dataset):
    ...
    def rotCrop(self, img, gt3Dcrop, com, cube,paras=None, normZeroOne=False):
        """
        Commonly used function to augment hand poses
        :param img: image
        :param gt3Dcrop: 3D annotations
        ...
        if np.max(img) == 0:
            ...
            imgD, new_joints3D, rot = self.rotateHand(img.astype('float32'), cube, com, rot, gt3Dcrop, paras,
                                                      pad_value=0)
        imgD = self.normalize_img(premax, imgD, com, cube)
        return imgD, None, new_joints3D, np.asarray(cube), com, None, rot
...
class AlohaDataset(loader):
    def __init__(self, root_dir, phase, aug_para=[10, 0.1, 180], img_size=128,
                 cube_size=[250, 250, 250], center_type='refine', joint_num=21, crop=False, color_factor=0, mode='RGB',train_file=None):
        super(AlohaDataset, self).__init__(root_dir, phase, img_size, center_type, 'aloha')
        ...
        # Path setup
        data_path = '{}/{}'.format(self.root_dir, self.phase)
        bubble_path = '{}/bubble_view'.format(data_path)
        label_path = '{}/kp/kp_json'.format(data_path)
        depth_path = '{}/depth_view'.format(data_path)  # Depth image path
        cprint(f'loading {self.phase} data from {data_path}...  MODE:{self.mode}', 'green')
        
        self.labels = label_path
        self.bubbles = bubble_path
        self.depths = depth_path  # Store depth path as well
        self.data_path = data_path
        self.crop = crop

        if self.crop:
            cprint(f'Dataset has been cropped!  MODE:{self.mode}', 'yellow')
        ...
        # Calculate crop boundaries (left, top, right, bottom)
        self.x1 = max(self.cx - self.half_crop, 0)
        self.y1 = max(self.cy - self.half_crop, 0)
        self.x2 = self.cx + self.half_crop
        self.y2 = self.cy + self.half_crop

        if train_file is not None:
            with open(train_file, 'r') as f:
                self.train_files = json.load(f)
        else:
            self.train_files = os.listdir(self.bubbles)

        cprint(f"Total number of {self.phase} images: {len(self.train_files)}", 'blue')
    ...
    def find_max_depth(self):
        """遍历所有深度图像，找到最大深度值"""
        max_depth = 0
        depth_files = os.listdir(self.depths)
        
        for depth_file in depth_files:
            # 只处理深度图文件
            if depth_file.endswith('.png'):
                depth_path = os.path.join(self.depths, depth_file)
                depth = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
                if isinstance(depth, np.ndarray):
                    max_depth = max(max_depth, np.max(depth))
                    
        print(f"Maximum depth value found: {max_depth}")
        return max_depth

    def __getitem__(self, idx):
        # Load RGB Image
        train_file = self.train_files[idx]
        img_path = os.path.join(self.bubbles, train_file)
        ...
        if self.crop:
            RGB = RGB[self.y1:self.y2, self.x1:self.x2]

        # Resize RGB image
        RGB = cv2.resize(RGB, (224, 224))  # Adjust size as needed

        # Load 3D Keypoint Data
        # label_path = self.labels + '/{:05d}_0_3d_kp.json'.format(idx)
        label_path = os.path.join(self.labels, f'{train_file.split(".")[0]}_0_3d_kp.json')

        with open(label_path, 'r') as f:
            label_data = json.load(f)
        
        joint_xyz = np.array(label_data['keypoints_3d'])
        ...
        if self.phase == 'train':
            ...
            imgRGB = self.transform(RGB.astype(np.float32)) / 255.

        # Initialize sample dictionary
        sample = {
            'image_rgb': imgRGB,
            'joint_xyz': torch.from_numpy(joint_xyz).float(),
        }

        # If mode is 'RGBD', load depth image as well
        if 'D' in self.mode:
            # Load Depth Image
            depth_path = os.path.join(self.depths, f'{train_file.split(".")[0]}.png')
            depth = cv2.imread(depth_path)[:, :, 0]  # Read depth image as single-channel
            if not isinstance(depth, np.ndarray):
                print("Depth read fail")
            ...
            # Normalize Depth Image using the maximum depth value found earlier
            depth = np.expand_dims(depth, axis=-1)  # Add channel dimension
            depth = self.transform(depth.astype(np.float32)) / self.max_depth

            sample['image_depth'] = depth  # Add depth image to the sample
            sample['pcl'] = depth  # Add depth as point cloud if needed
    ...
    def show_cropped_image(self, cropped_RGB):
        """Debugging function to show the cropped RGB image (only RGB)"""
...
class GloveDataset(loader):
    def __init__(self, root_dir, phase, aug_para=[10, 0.1, 180], img_size=128,
                 cube_size=[250, 250, 250], center_type='refine', joint_num=12, crop=False, color_factor=0, mode='RGB',train_file=None):
        super(GloveDataset, self).__init__(root_dir, phase, img_size, center_type, 'aloha')
        ...
        self.transform = transforms.ToTensor()
        self.mode = mode
        self.keys_order = ['l2', 'l0', 'l1', 'l4', 'l5', 'l7', 'l8', 'l9', 'l12', 'l13', 'l16', 'l17']

        # Path setup
        data_path = '{}/{}'.format(self.root_dir, self.phase)
        bubble_path = '{}/bubble_view'.format(data_path)
        label_path = '{}/json'.format(data_path)
        depth_path = '{}/depth_view'.format(data_path)  # Depth image path
        cprint(f'loading {self.phase} data from {data_path}...  MODE:{self.mode}', 'green')
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.bubbles, img_name)
        # img_path = self.bubbles + '/each_glove_{:05d}.jpg'.format(idx)
        RGB = cv2.imread(img_path)
        if not isinstance(RGB, np.ndarray):
            print("RGB read fail")
            RGB = np.zeros(shape=(480, 640, 3), dtype=np.uint8)
        ...
        # Load 3D Keypoint Data
        # label_path = self.labels + '/each_glove_{:05d}.json'.format(idx)
        label_path = os.path.join(self.labels, img_name.replace('.jpg', '.json'))
        with open(label_path, 'r') as f:
            label_data = json.load(f)
    
        # Create a 12-dimensional vector (NumPy array)
        joint_rad = np.array([label_data[key] for key in self.keys_order])
        joint_rad = torch.from_numpy(joint_rad).float()/100
        # Data Augmentation for training phase
        if self.phase == 'train':
            imgRGB = RGB
            if self.color_factor != 0:
        ...
        sample = {
            'image_rgb': imgRGB,
            'joint_rad': joint_rad.unsqueeze(-1),
        }

        # If mode is 'RGBD', load depth image as well
        if 'D' in self.mode:
            # Load Depth Image
            depth_path = self.depths + '/{:05d}.jpg'.format(idx)
            depth = cv2.imread(depth_path)[:, :, 0]  # Read depth image as single-channel
            if not isinstance(depth, np.ndarray):
                print("Depth read fail")
                depth = np.zeros(shape=(480, 640), dtype=np.uint16)
