...
class OfficialResNetUnet(nn.Module):
    def __init__(self, backbone, joint_num, pretrain=True, deconv_dim=128, out_dim_list=[3*21, 21, 21]):
        ...
        self.conv_to_joints = nn.Conv2d(in_channels=sum(out_dim_list), out_channels=self.joint_num * 3, kernel_size=1, stride=1)
    ...
    def forward(self, img):
        ...
        img_result = self.conv_to_joints(img_result)  # (B, 63, H, W)

        # Apply global average pooling to reduce spatial dimensions (H, W)
        img_result = nn.functional.adaptive_avg_pool2d(img_result, (1, 1))  # (B, 63, 1, 1)

        # Flatten the result to get (B, 63)
        img_result = img_result.view(img_result.size(0), -1)  # (B, 63)

        # Reshape to (B, 21, 3) as required
        img_result = img_result.view(-1, 21, 3)

        return img_result, img_feature


class OfficialResNetUnet_RGB2offset_3D(nn.Module):
    def __init__(self, backbone, joint_num, pretrain=True, deconv_dim=128, out_dim_list=[3*21+21+21],joint_out_dim = 3):
        ...
        self.conv_to_joints = nn.Conv2d(in_channels=sum(out_dim_list), out_channels=self.joint_num * joint_out_dim, kernel_size=1, stride=1)

        self.fc_to_joints = None
    ...
    def forward(self, img):
        ...
        # Reshape to (B, 21, 3) as required
        img_result = img_result.view(-1, self.joint_num, self.joint_out_dim )
        
        return img_result, img_feature

class OfficialResNetUnet_RGB2offset_3D_FC(nn.Module):
    def __init__(self, backbone, joint_num, pretrain=True, deconv_dim=128, out_dim_list=[3*21, 21, 21]):
        super(OfficialResNetUnet_RGB2offset_3D, self).__init__()
        self.joint_num = joint_num
        self.feature_dim = [self.joint_num * 3, self.joint_num]
        ...
        self.fc_to_joints = nn.Linear(4872000, joint_num * 3, bias=False)
    ...
    def forward(self, img):
        ...
        img_result = img_result.view(img_result.size(0), -1) 
        img_result = self.fc_to_joints(img_result) 
        img_result = img_result.view(-1, 21, 3) 
