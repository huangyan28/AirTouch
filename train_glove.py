...
from model.model import RGBmodel,RGBDmodel,Glovemodel
...
class Trainer(object):
    def __init__(self, config):
        ...
        self.net = Glovemodel(self.config.net, self.config.pretrain, self.joint_num, self.dataset,
        ...
        if self.config.dataset == 'dexycb':
            ...
            self.evalLoader = DataLoader(self.evalData, batch_size=1, shuffle=False,
                                         num_workers=8)
        elif self.config.dataset == 'glove':
            if self.config.phase == 'train':
                # print(self.config.augment_para)

                self.trainData = loader.GloveDataset(self.config.root_dir, 'train', self.config.augment_para,
                                     
                                     img_size=self.config.input_size,mode=self.config.mode,crop=self.config.crop,train_file = self.config.train_file)
                self.trainLoader = DataLoader(self.trainData, batch_size=self.config.batch_size, shuffle=True,
                                            num_workers=8)
            self.testData = loader.GloveDataset(self.config.root_dir, 'test', self.config.augment_para,
                                img_size=self.config.input_size,mode=self.config.mode,crop=self.config.crop)
            self.testLoader = DataLoader(self.testData, batch_size=self.config.batch_size, shuffle=False,
                                            num_workers=8)
            self.evalData = loader.GloveDataset(self.config.root_dir, 'evaluation', self.config.augment_para,
                                            img_size=self.config.input_size,mode=self.config.mode,crop=self.config.crop)
            self.evalLoader = DataLoader(self.evalData, batch_size=1, shuffle=False,
                                         num_workers=8)
        else:
    ...
    def test(self, epoch=-1):
        ...
        for ii, data in tqdm(enumerate(self.testLoader)):
            ...
            for index, stage_type in enumerate(self.config.stage_type):
                ...
                joint_rad = pixel_pd
                joint_errors = self.xyz2error(joint_rad, xyz_gt, self.result_file_list[index])
                batch_errors = np.mean(joint_errors, axis=-1)

                pck_errors_01 = self.pck(joint_rad, xyz_gt, threshold=0.01)
                pck_errors_0075 = self.pck(joint_rad, xyz_gt, threshold=0.0075)
                pck_errors_005 = self.pck(joint_rad, xyz_gt, threshold=0.005)
                pck_errors_0025 = self.pck(joint_rad, xyz_gt, threshold=0.0025)
                ...
                # for b in range(joint_rad.shape[0]):

                #     j, j_gt = joint_rad[b].cpu().numpy(), xyz_gt[b].cpu().numpy()
                #     if self.dataset != 'STB':
                #         joint_rad_aligned = self.GFM_.rigid_align(j, j_gt)
                #     else:
                #         joint_rad_aligned = joint_rad - (joint_rad[0] - xyz_gt[0])

                #     joint_errors_aligned += self.xyz2error(torch.from_numpy(joint_rad_aligned).cuda().unsqueeze(0),
                #                                             xyz_gt[b].unsqueeze(0), self.result_file_list[index])
                # batch_errors_aligned = joint_errors_aligned / joint_rad.shape[0]
        ...
        for index in range(len(error_list)):
            ...
            # error_info += ' error' + str(index) + ": %.6f" % (PA_error_list[index] / batch_num) + ' '
                        # 输出 PCK 到 log
    ...
    def evalution(self, epoch=-1):
        ...
        for ii, data in tqdm(enumerate(self.evalLoader)):
            ...
            for index, stage_type in enumerate(self.config.stage_type):
                ...
                joint_rad = results[index]

                joint_rad_list.append(joint_rad.cpu().detach().numpy()) 

            # print(f'joint_rad {joint_rad[0][0]}')
            joint_rad_array = np.concatenate(joint_rad_list, axis=0)
            # print(f'joint_rad_array{joint_rad_array}')
            # print(f'xyz_gt {xyz_gt[0][0]}')
            joint_list = joint_list + np.split(joint_rad_array, batch_size, axis=0)
    ...
    def z2error(self, output, joint, center, cube_size, write_file=None):
        ...
        joint_rad = output * cube_size / 2 + center
        joint_world_select = joint * cube_size / 2 + center

        # errors = (joint_rad - joint_world_select) * (joint_rad - joint_world_select)
        # errors = np.sqrt(np.sum(errors, axis=2))
        # print(joint_rad.shape)
        depth_errors = abs((joint_rad - joint_world_select)[:, :, 2])
        return depth_errors
    ...
    def xy2error(self, output, joint, center, cube_size, write_file=None):
        ...
        xy_errors = (joint_rad - joint_world_select)[:, :, :2] * (joint_rad - joint_world_select)[:, :, :2]
